...
if draft is None:
    ...
    draft = {
        ...
        "court_rate_1": 150, "court_hours_1": 0.0,
...
with tab_check:
    ...
    if st.session_state.check_groups:
        ...
        st.progress(checked_count / total_p_count)
...
with tab_main:
    ...
    with col_c1_hr:
        ...
        court_hours_1 = st.number_input("使用小時數", min_value=0, value=float(draft.get("court_hours_1", 1)), step=1, key="c_hours_1", on_change=sync_state_to_draft)
    ...
    if not st.session_state.ball_types:
        ...
        total_ball_fee = math.floor(exact_ball_fee)
