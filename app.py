...
def update_history_record(record_id, revenue, expense, profit, note):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("UPDATE history SET revenue=?, expense=?, profit=?, note=? WHERE id=?", 
                   (int(revenue), int(expense), int(profit), note, record_id))
    conn.commit()
    conn.close()
...
with tab_history:
    ...
    if not df.empty:
        st.subheader("📈 歷史利潤趨勢")
        st.bar_chart(df.set_index("日期")["總利潤"])
